...
```
oc delete all,secrets,sa,configmaps,daemonsets,clusterroles,rolebindings,serviceaccounts --selector=template=infinispan-ephemeral || true
```

You can also remove individual Infinispan Server clusters by using a `--selector` parameter that matches the cluster's application name.
...
A small `Makefile` is provided in the repository for managing templates and instances created by these templates.
